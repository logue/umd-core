...
- [高度なUMD機能](#高度なumd機能) ✅ **実装済み** (2026/03/03)
  - [~~数式サポート~~](#数式サポートmath-formula-support) ✅ **実装済み** (2026/03/03)
  - [~~ポップオーバー~~](#ポップオーバーpopover) ✅ **実装済み** (2026/03/03)
...
> ✅ **実装済み** (2026年3月3日)  
> この機能は実装が完了しました。詳細は [docs/implemented-features.md#インラインコードのカラーサンプル](implemented-features.md#インラインコードのカラーサンプル) を参照してください。

インラインコードの内容がカラーコード（HEX/RGB/RGBA/HSL/HSLA）に一致する場合、`<code>` の末尾に色見本を自動付与します。
...
- 実装では `&math(...)`（インライン）と `@math(...)` / `@math(){{...}}`（ブロック）をライブラリ側でMathMLへ直接変換
- 変換には `math-core` を使用
- 実装箇所: [src/extensions/conflict_resolver.rs](../src/extensions/conflict_resolver.rs)
...
詳細仕様は [docs/template-engine-spec.md](template-engine-spec.md) を参照してください。
...
参照スタイルリンクは文法的な直感性の観点から保留とします。

**保留理由**:

1. Markdownの基本的なリンク記法は`[テキスト](URL)`であり、`[テキスト][ref]`は初見で意図が伝わりにくい
2. `[][]`に近い視覚構造は、記法の学習コストを上げる可能性がある
3. UMDでは読み書き時の明確さを優先し、まずはインラインリンク中心で運用する
...
Setext見出し（下線形式の見出し）は**サポートしません**。

**理由**:

- 自動整形処理で通常のATX見出し（`#`, `##`）へ変換されるため、記法としての独立性を維持できない
- 表記揺れを増やすより、見出し記法をATX形式へ統一したほうが運用が明確

### インライン数式

`$...$` を使ったインライン数式表記は**サポートしません**。

**理由**:

- 通貨記号や一般テキストの`$`と衝突しやすく、誤検出を招きやすい
- UMDでは数式は `&math(...)` / `@math(...)` に統一し、記法の曖昧さを避ける
