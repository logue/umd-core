...
特に `//` はコメント記法として解釈されず、コードの一部としてそのまま表示されます。
...
```css
.spoiler {
  background-color: var(--bs-secondary-bg, #202225);
  color: transparent;
  cursor: pointer;
...
.spoiler:active,
.spoiler.revealed {
  background-color: var(--bs-secondary-bg-subtle, #2f3136);
...
競合解決ステップ（Step 4）後、Markdownソーステキストから以下の文字を除去します:
...
**保持される文字**: `U+0009`（TAB）、`U+000A`（LF）、`U+000D`（CR）はMarkdown書式に必要なため保持。

**除外対象（除去しない領域）**:

- **フェンスコードブロック**（` ``` ` / `~~~` 内）: ターミナルエスケープシーケンスなど、意図的な制御文字を含む可能性があるため除外
- **プラグインコンテンツ**: Step 4でbase64エンコードされるため、制御文字はマーカー変換前に処理される。プラグインコンテンツのサニタイズは**プラグイン作者の責任**（後述）

#### プラグインコンテンツのサニタイズ責任

プラグイン（`@fn(args){{ ... }}` / `&fn(args){...};`）のコンテンツは、競合解決時にbase64エンコードされた状態でマーカー化されます。そのため、プラグインコンテンツ中の制御文字はそのままbase64として保存され、パーサーのHTMLエスケープ対象となりますが、**バックエンドでのレンダリング時には制御文字がそのまま渡される**可能性があります。

> **注意**: プラグインのコンテンツをサーバーサイドでレンダリングするプラグイン作者は、受け取ったコンテンツに対して適切なサニタイズを実施する必要があります。これはUMDパーサーのスコープ外の責任です。
