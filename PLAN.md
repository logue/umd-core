...
- ✅ ブロック型プラグイン書式の拡張（`::: 記法`）
- ✅ プラグイン記法（`&`/`@`）のネスト・保護処理をバランス括弧スキャン方式に刷新（2026年9月。ネスト時のマーカー破損・引数中の`)`でマッチ失敗するバグを修正。詳細はCHANGELOG.mdと[docs/runtime-features.md](docs/runtime-features.md)の「プラグイン構文の深いネスト・content/argsの扱い」参照）
